...
def start_game():
    ...
    date=s.recv(1024).decode("utf-8")
    my_buildings=json.loads(date) #словник інформації про будівлі
...
class Game(Screen): # ігровий клас
    ...
    def __init__(self): # конструктор класу
        ...
        self.all_resours={"people":"-","food":"-","tree":"-","stone":"-",
        "oil":"-","iron":"-","gold":"-"}
        ...
        self.oil_button_text=Button(size_hint=[1,0.08],text="-",background_normal="", background_down="",color=[1,0,1,1],background_color=[0,0,0,1])
    ...
    def update(self,clock):
        if all_command:
            ...
            if command["action"]=="reboot" or command["action"]=="update_resource":
                for res in self.all_resours:
                    self.all_resours[res]=command[res]  
                print(self.all_resours)  
        if resource_mining_speed:
            ...
            for res in resource_time_mining_speed:
                if time_now-resource_time_mining_speed[res]>=resource_mining_speed[res]:
                    self.all_resours[res]+=1
                    resource_time_mining_speed[res]=time_now+ time_now-resource_time_mining_speed[res]-resource_mining_speed[res]
                    #print(self.all_resours[res])
            self.people_button_text.text=str(self.all_resours["people"])
            self.tree_button_text.text=str(self.all_resours["tree"])
            self.iron_button_text.text=str(self.all_resours["iron"])
            self.stone_button_text.text=str(self.all_resours["stone"])
            self.oil_button_text.text=str(self.all_resours["oil"])
            self.gold_button_text.text=str(self.all_resours["gold"])
            self.food_button_text.text=str(self.all_resours["food"]) 
