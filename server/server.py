    ...
    if command["action"]=="autorization":
        ...
        if command["token"]+"\n" in all_token:
            ...
            for build in list_buildings:
                cursor.execute("INSERT INTO {build} (id_user, count, last_update, new_buildings_count) VALUES (%s, %s, %s, %s)", (user_id,0,time_now,0))
            for army in list_buildings:
                cursor.execute("INSERT INTO {army} (id_user, count, last_update, new_army_count) VALUES (%s, %s, %s, %s)", (user_id,0,time_now,0))
